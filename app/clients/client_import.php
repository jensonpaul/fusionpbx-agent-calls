...
//save the data to the csv file
	if (isset($_POST['data'])) {
		$file = $_SESSION['server']['temp']['dir']."/clients-".$_SESSION['domain_name'].".csv";
...
//copy the csv file
	//$_POST['submit'] == "Upload" &&
	if ( is_uploaded_file($_FILES['ulfile']['tmp_name']) && permission_exists('client_upload')) {
		if ($_POST['type'] == 'csv') {
...
//get the schema
	if (strlen($delimiter) > 0) {
		...
		//get the schema
			$x = 0;
			include "app/clients/app_config.php";
			$i = 0;
	...
	if (strlen($delimiter) > 0 && file_exists($_SESSION['file']) && $action != 'import') {
		...
		//validate the token
			...
			if (!$token->validate($_SERVER['PHP_SELF'])) {
				...
				header('Location: client_import.php');
				exit;
			}
		...
		//include header
			$document['title'] = $text['title-clients_import'];
		...
		//form to match the fields to the column names
			...
			echo "	<div class='heading'><b>".$text['header-clients_import']."</b></div>\n";
			echo "	<div class='actions'>\n";
			echo button::create(['type'=>'button','label'=>$text['button-back'],'icon'=>$_SESSION['theme']['button_icon_back'],'id'=>'btn_back','style'=>'margin-right: 15px;','link'=>'client_import.php']);
			echo button::create(['type'=>'submit','label'=>$text['button-import'],'icon'=>$_SESSION['theme']['button_icon_import'],'id'=>'btn_save']);
			echo "	</div>\n";
			echo "	<div style='clear: both;'></div>\n";
			echo "</div>\n";

			echo $text['description-clients_import']."\n";
			echo "<br /><br />\n";
							...
							//format the data
								...
								foreach ($fields as $key => $value) {
									...
									if (strlen($table_name) > 0) {
										if (strlen($parent) == 0) {
											$array[$table_name][$row_id]['client_uuid'] = uuid();
											$array[$table_name][$row_id][$field_name] = $result[$key];
										}
										else {
											$array[$parent][$row_id][$table_name][$y]['client_uuid'] = uuid();
